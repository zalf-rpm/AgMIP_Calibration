...
observations = "training_Zadoks_dates_v2.txt"
...
with open(crop_sim_site_MAP) as exp_mapfile:
    ...
    for row in reader:
        ...
        exp_map["species_file"] = basepath+"\\param_files\\crops\\"+row[5]
        exp_map["cultivar_file"] = basepath+"\\param_files\\crops\\wheat\\"+row[6]
...
with open(observations) as obsfile:
    ...
    for row in reader:
        ...
        for i in range(len(header)):
            my_var = header[i]
            if my_var in use_zadoks and row[i] != "NA":                
                record = {}
                record["exp_ID"] = exp_id
                record["stage"] = my_var
                obs_date = row[i].split("/")
                my_date = date(int(obs_date[2]), int(obs_date[1]), int(obs_date[0]))
                record["date"] = my_date
                record["DOY"] = my_date.timetuple().tm_yday                  
